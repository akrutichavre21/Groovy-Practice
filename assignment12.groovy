...
List groupList = [[name:"OFFICE"], [name:"HOME"], [name:"GYM"], [name:"WORK"], [name:"MARKET"]]

groupList.each { item ->
    Group groupInstance = new Group(item)
    ...
    switch (choice) {
        ...
        case 1: contactMap.firstName = System.console().readLine "Enter First Name > "
            ...
            while (wrongGroup) {
                ...
                if ((groupName == "OFFICE") || (groupName == "HOME") || (groupName == "GYM") || (groupName == "WORK") || (groupName == "MARKET")) {
                    wrongGroup = false
                    break
                }
            ...
            groupList.eachWithIndex { item, index ->
                if (item.name == groupName) {
                    groupInstances[index].contacts.push(myContact)
                    println "${groupInstances[index].name}"
                    println "${groupInstances[index].contacts.firstName}"
                }
            }
